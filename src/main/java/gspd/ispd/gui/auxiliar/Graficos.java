...
import gspd.ispd.motor.metricas.MetricasComunicacao;
import gspd.ispd.motor.metricas.MetricasProcessamento;
...
public class Graficos {
    ...
    public void criarProcessamentoTempoTarefa(List<Tarefa> tarefas) {
        ...
        if (!tarefas.isEmpty()) {
            for (Tarefa task : tarefas) {
                XYSeries tmp_series;
                tmp_series = new XYSeries(task.toString());
                ...
                if (temp != null) {
                    ...
                    Double last = 0.0;
                    //System.out.println(task+" : "+task.getLocalProcessamento().getId());
                    for (int j = 0; j < task.getTempoInicial().size(); j++) {
                        //System.out.print("inicio "+task.getTempoInicial().get(j));
                        //System.out.println(" fim "+task.getTempoFinal().get(j));
                        if (last <= task.getTempoInicial().get(j)) {
        ...
        TaskThroughTimeChart.setPreferredSize(new Dimension(600, 300));
    }
            ...
            for (CS_Processamento maq : rdf.getMaquinas()) {
                ...
                if (!lista.isEmpty()) {
                    ...
                    for (int i = 0; i < lista.size(); i++) {
                        ...
                        tmp_series.add(lista.get(i).getInicio(), (Number) 0.0);
                        tmp_series.add(lista.get(i).getInicio(), uso);
                        //Adiciona ponto final.
                        int j = i + 1;
                        while (j < lista.size() && lista.get(i).getFim() >= lista.get(j).getInicio()) {
                            j++;
                            i++;
                        }
                        tmp_series.add(lista.get(i).getFim(), uso);
                        tmp_series.add(lista.get(i).getFim(), (Number) 0.0);
                    }
                    //Add no gráfico.
                    dadosGrafico.addSeries(tmp_series);
                }
    ...
    public ChartPanel criarGraficoNumTarefasAproveitamento(List<Tarefa> tarefas) {
        ...
        DefaultCategoryDataset dadosMflopProcessados = new DefaultCategoryDataset();
        int numExcesso = 0, numOK = 0;
        int i;

        for (i = 0; i < tarefas.size(); i++) {

            if (tarefas.get(i).getEstado() != Tarefa.CANCELADO && tarefas.get(i).getMflopsDesperdicados() != 0.0) {
                numExcesso++;
            } else {
                numOK++;
            }
